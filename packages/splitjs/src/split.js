...
const Split = (idsOption, options = {}) => {
    ...
    const gutterStyle = getOption(options, 'gutterStyle', defaultGutterStyleFn)
    const collapsed = getOption(options, 'collapsed', [false, false]);
    // 2. Initialize a bunch of strings based on the direction we're splitting.
    // A lot of the behavior in the rest of the library is paramatized down to
    // rely on CSS strings and classes.
    if (direction === HORIZONTAL) {
    ...
    function collapse(idx) {
        ...
        element.element.style.display = "hidden";
        ...
        setElementSize(elements[0].element, elements[0].i == idx ? 0 : 100 - (pairs[0][aGutterSize]/pair.size * 100), pairs[0][aGutterSize], elements[0].i);
        setElementSize(elements[1].element, elements[1].i == idx ? 0 : 100 - (pairs[0][bGutterSize]/pair.size * 100), pairs[0][bGutterSize], elements[1].i);
    }

    function uncollapse(idx) {
        var element = elements[idx];
        element.collapsed = false;
        element.element.style.display = "";

        setElementSize(elements[0].element, elements[0].size, pairs[0][aGutterSize], elements[0].i);
        setElementSize(elements[1].element, elements[1].size, pairs[0][bGutterSize], elements[1].i);
    }
